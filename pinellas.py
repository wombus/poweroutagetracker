...
# Set the URL
url = 'https://poweroutage.us/area/county/336'
...
while True:
    ...
    if previous_customers_out is not None:
        if customers_out_value > previous_customers_out:
            cust_diff = int(customers_out_value) - int(previous_customers_out)
            color = Fore.RED  # More outages, color the line red
        elif customers_out_value < previous_customers_out:
            cust_diff = int(previous_customers_out) - int(customers_out_value)
    ...
    print(f"[Pinellas] {color}[{str(n_reqs)}]Customers Tracked: {customers_tracked} | "
